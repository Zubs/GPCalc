        ...
        <div class="form-group">
          ...
          <div class="pw-strength" v-if="form.password">
            <div class="strength-bars">
              <div v-for="n in 4" :key="n" class="strength-bar" :class="{ active: n <= strength.level, [`level-${strength.level}`]: n <= strength.level }"></div>
            </div>
...
const form = reactive({ name: '', email: '', institution: '', scale: '4.0', password: '' })
...
async function handleRegister() {
  ...
  loading.value = true
  try {
    await auth.register(form.name, form.email, form.password, form.institution)
...
.strength-bar {
  ...
  background: var(--bg-overlay); transition: background 0.2s;
}
.strength-bar.active.level-1 { background: var(--red-500); }
.strength-bar.active.level-2 { background: #f59e0b; }
.strength-bar.active.level-3 { background: #84cc16; }
.strength-bar.active.level-4 { background: var(--green-500); }
.strength-label { font-size: 0.75rem; color: var(--text-muted); }
.strength-label.level-1 { color: var(--red-500); }
.strength-label.level-2 { color: #f59e0b; }
.strength-label.level-3 { color: #84cc16; }
.strength-label.level-4 { color: var(--green-500); }
.auth-error {
  display: flex; align-items: center; gap: 8px;
  background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.2);
  color: var(--red-500); border-radius: var(--radius-md);
  padding: 10px 12px; font-size: 0.85rem;
}
.terms-note { font-size: 0.78rem; color: var(--text-muted); text-align: center; line-height: 1.5; }
.terms-note a { color: var(--accent); }
.auth-switch { margin-top: 20px; text-align: center; font-size: 0.87rem; color: var(--text-muted); }
.auth-link { color: var(--accent); font-weight: 500; }
.auth-link:hover { text-decoration: underline; }
.loading-dots { display: flex; gap: 4px; align-items: center; }
.loading-dots span { width: 5px; height: 5px; border-radius: 50%; background: currentColor; animation: pulse 1.2s ease-in-out infinite; }
