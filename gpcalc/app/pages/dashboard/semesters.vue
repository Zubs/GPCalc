      ...
      <div v-for="year in academic.years" :key="year.id" class="year-block">
        <div class="year-header">
          <div class="year-info">
            ...
            <span class="year-stats">{{ year.semesters.length }} semester{{ year.semesters.length !== 1 ? 's' : '' }}</span>
          </div>
        ...
        <div v-if="!year.semesters.length" class="no-sems">
          No semesters yet. <button class="inline-link" @click="openAddSemester(year.id)">Add one →</button>
        </div>

        <div class="semesters-grid">
          <div v-for="sem in year.semesters" :key="sem.id" class="semester-card" :class="{ expanded: expandedSem === sem.id }">
            <div class="sem-card-header" @click="toggleSem(sem.id)">
              ...
              <div class="sem-card-actions">
                <button class="btn-icon danger" title="Remove semester" @click.stop="academic.removeSemester(year.id, sem.id)">
                  ...
                  <table class="courses-table">
                    ...
                    <tbody>
                      <tr v-for="course in sem.courses" :key="course.id" class="course-row">
                        ...
                        <td class="num">
                          <input v-model.number="course.credits" type="number" min="1" max="12" class="cell-input num-input"
                            @change="saveCourse(year.id, sem.id, course)" />
                        </td>
                        <td class="num">
                          <select v-model="course.grade" class="cell-select" @change="onGradeChange(year.id, sem.id, course)">
                            <option value="">—</option>
                            <option v-for="g in academic.scaleGrades" :key="g.grade" :value="g.grade">{{ g.grade }}</option>
                          </select>
                        </td>
                        <td class="num">
                          <span class="points-val" :class="gpaClass(course.gradePoints / academic.maxPoints * academic.maxPoints)">
...
function addYear() {
  if (!newYearLabel.value.trim()) return
...
<style scoped>
.semesters-page { display: flex; flex-direction: column; gap: 28px; }
...
.empty-title { font-size: 1.6rem; margin-bottom: 8px; }
.empty-sub { color: var(--text-muted); font-size: 0.9rem; margin-bottom: 28px; }
...
.year-info { display: flex; align-items: baseline; gap: 12px; }
.year-label { font-size: 1.05rem; font-weight: 500; }
.year-stats { font-size: 0.78rem; color: var(--text-muted); }
.year-actions { display: flex; gap: 8px; align-items: center; }

.no-sems { padding: 24px; font-size: 0.88rem; color: var(--text-muted); }
.inline-link { background: none; border: none; color: var(--accent); cursor: pointer; font-size: inherit; }

/* Semesters grid */
.semesters-grid { display: flex; flex-direction: column; gap: 0; }

.semester-card { border-bottom: 1px solid var(--border); }
.semester-card:last-child { border-bottom: none; }
...
.sem-card-header:hover { background: var(--bg-elevated); }

.sem-card-info { display: flex; flex-direction: column; gap: 4px; }
.sem-name-row { display: flex; align-items: center; gap: 10px; }
.sem-season-badge {
...
.sem-card-name { font-size: 0.95rem; font-weight: 500; }
.sem-card-stats { font-size: 0.78rem; color: var(--text-muted); display: flex; align-items: center; gap: 6px; }
.dot-sep { opacity: 0.4; }
.sem-gpa-inline { font-weight: 500; }
.sem-gpa-inline.high { color: var(--green-400); }
.sem-gpa-inline.mid  { color: var(--amber-400); }
.sem-gpa-inline.low  { color: var(--red-500); }

.sem-card-actions { display: flex; align-items: center; gap: 8px; }
.chevron { color: var(--text-muted); transition: transform 0.2s; }
.semester-card.expanded .chevron { transform: rotate(180deg); }

/* Courses */
.sem-courses { padding: 0 24px 20px; border-top: 1px solid var(--border); background: var(--bg-base); }
.courses-table-wrap { overflow-x: auto; margin-top: 16px; }
.courses-table { width: 100%; border-collapse: collapse; min-width: 560px; }
.courses-table th {
  font-size: 0.73rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em;
  color: var(--text-muted); padding: 8px 10px; text-align: left;
  border-bottom: 1px solid var(--border);
}
.courses-table th.num { text-align: center; }
.course-row { transition: background 0.1s; }
.course-row:hover { background: var(--bg-elevated); }
.course-row td { padding: 6px 10px; border-bottom: 1px solid var(--border); }

.cell-input {
  background: transparent; border: 1px solid transparent;
  border-radius: var(--radius-sm); padding: 5px 8px;
  color: var(--text-primary); font-size: 0.86rem; width: 100%;
  transition: border-color 0.15s, background 0.15s;
}
.cell-input:focus { outline: none; border-color: var(--accent); background: var(--bg-elevated); }
.code-input { font-family: monospace; font-size: 0.82rem; }
...
.points-val { font-size: 0.88rem; font-weight: 500; }
.points-val.high { color: var(--green-400); }
.points-val.mid  { color: var(--amber-400); }
.points-val.low  { color: var(--red-500); }

.courses-footer { display: flex; justify-content: space-between; align-items: center; margin-top: 12px; }
.sem-summary { display: flex; align-items: center; gap: 10px; }
.sum-label { font-size: 0.8rem; color: var(--text-muted); }
.sum-val { font-size: 1.1rem; font-weight: 600; font-family: var(--font-display); }
.sum-val.high { color: var(--green-400); }
.sum-val.mid  { color: var(--amber-400); }
.sum-val.low  { color: var(--red-500); }
...
.btn-icon:hover { border-color: var(--border-strong); color: var(--text-secondary); background: var(--bg-elevated); }
.btn-icon.danger:hover { border-color: var(--red-500); color: var(--red-500); background: rgba(239,68,68,0.08); }

/* Expand transition */
.expand-enter-active, .expand-leave-active { transition: all 0.2s var(--ease); overflow: hidden; }
...
.season-buttons { display: flex; gap: 8px; margin: 20px 0; }
.season-btn {
...
.season-btn.active { background: var(--accent-subtle); border-color: var(--amber-400); color: var(--amber-400); }
.season-btn:hover:not(.active) { background: var(--bg-elevated); }
</style>
