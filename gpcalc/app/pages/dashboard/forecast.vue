      ...
      <div class="forecast-result-col">
        <div class="result-card" :class="resultStatus">
          <template v-if="!result">
            <div class="result-empty">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.2" opacity="0.3"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>
            ...
            <div class="grade-guide">
              ...
              <div v-for="g in gradeGuide" :key="g.grade" class="gg-row">
                ...
                <div class="gg-bar-wrap">
                  <div class="gg-bar" :style="{ width: (g.points / academic.maxPoints * 100) + '%', background: g.points >= result ? 'var(--green-500)' : 'var(--red-500)' }"></div>
                </div>
        ...
        <div class="card scenarios-card" v-if="targetCGPA && upcomingCredits">
          ...
          <div class="scenarios-table">
            ...
            <div v-for="scen in scenarios" :key="scen.upcoming" class="scen-row" :class="{ target: scen.upcoming === result }">
...
const result = computed(() => {
  if (!targetCGPA.value || !upcomingCredits.value) return null
  return academic.forecastRequiredGPA(+targetCGPA.value, +upcomingCredits.value)
})

const resultStatus = computed(() => {
  if (!result.value) return ''
  if (result.value > academic.maxPoints) return 'status-impossible'
  if (result.value <= 0) return 'status-achieved'
  if (result.value >= academic.maxPoints * 0.9) return 'status-stretch'
...
function classify(cgpa: number) {
  ...
  if (pct >= 0.8) return '2nd Class Upper'
  if (pct >= 0.7) return '2nd Class Lower'
...
const targetBadge = computed(() => {
  ...
  if (c === 'First Class') return 'badge-green'
  if (['2nd Class Upper', 'Upper Second'].includes(c)) return 'badge-amber'
  return 'badge-muted'
})
...
const scenarios = computed(() => {
  if (!targetCGPA.value || !upcomingCredits.value) return []
  ...
  return Array.from({ length: 11 }, (_, i) => {
    ...
    return { upcoming: upcomingGPA, cgpa, classification: classify(cgpa) }
  }).reverse()
})
</script>
...
.current-stat { flex: 1; min-width: 120px; display: flex; flex-direction: column; gap: 4px; padding: 0 20px; }
.current-stat:first-child { padding-left: 0; }
.cs-label { font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.06em; font-weight: 500; }
.cs-val { font-size: 1.6rem; font-weight: 600; font-family: var(--font-display); color: var(--text-primary); }
.cs-val.amber { color: var(--amber-400); }
.cs-val.sm { font-size: 1rem; }
.current-divider { width: 1px; background: var(--border); align-self: stretch; flex-shrink: 0; }

/* Body */
.forecast-body { display: grid; grid-template-columns: 380px 1fr; gap: 20px; }
.card { background: var(--bg-surface); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 24px; }
.card-title { font-size: 1rem; font-weight: 500; margin-bottom: 20px; }
.card-sub { font-size: 0.82rem; color: var(--text-muted); margin-top: -14px; margin-bottom: 16px; }

.forecast-form-col { display: flex; flex-direction: column; gap: 16px; }
.forecast-form-col .card { display: flex; flex-direction: column; gap: 18px; }

/* Target input */
.target-input-wrap { display: flex; align-items: center; gap: 10px; }
.target-input { font-size: 1.5rem; font-weight: 600; font-family: var(--font-display); }
.target-max { font-size: 1.1rem; color: var(--text-muted); white-space: nowrap; }
.target-classification { display: flex; align-items: center; gap: 8px; margin-top: 4px; }

/* Quick targets */
.qt-label { font-size: 0.75rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.06em; color: var(--text-muted); margin-bottom: 10px; }
.qt-buttons { display: flex; gap: 8px; flex-wrap: wrap; }
.qt-btn {
...
.qt-btn:hover { border-color: var(--amber-400); background: var(--accent-subtle); }
.qt-btn.active { border-color: var(--amber-400); background: var(--accent-subtle); }
.qt-val { font-size: 1rem; font-weight: 600; color: var(--amber-400); font-family: var(--font-display); }
.qt-name { font-size: 0.68rem; color: var(--text-muted); white-space: nowrap; }
...
.result-empty { display: flex; flex-direction: column; align-items: center; gap: 12px; color: var(--text-muted); font-size: 0.88rem; text-align: center; }

.result-label { font-size: 0.82rem; color: var(--text-muted); }
.result-gpa { font-size: 4rem; font-weight: 300; font-family: var(--font-display); color: var(--text-primary); line-height: 1; }
.result-sublabel { font-size: 0.85rem; color: var(--text-muted); }

.status-impossible .result-gpa { color: var(--red-500); }
.status-achieved .result-gpa   { color: var(--green-400); }
.status-stretch .result-gpa    { color: var(--amber-400); }
.status-reachable .result-gpa  { color: var(--green-400); }

.result-status-row { margin-top: 4px; }
.status-badge { display: inline-flex; padding: 6px 12px; border-radius: var(--radius-md); font-size: 0.82rem; font-weight: 500; }
.status-badge.success { background: rgba(34,197,94,0.1);  color: var(--green-400); }
.status-badge.warning { background: var(--accent-subtle); color: var(--amber-400); }
.status-badge.danger  { background: rgba(239,68,68,0.1);  color: var(--red-500); }

/* Grade guide */
.grade-guide { margin-top: 12px; padding-top: 16px; border-top: 1px solid var(--border); }
.gg-title { font-size: 0.78rem; color: var(--text-muted); font-weight: 500; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.05em; }
.gg-row { display: flex; align-items: center; gap: 10px; margin-bottom: 6px; }
.gg-grade { width: 60px; font-size: 0.8rem; color: var(--text-secondary); flex-shrink: 0; }
.gg-bar-wrap { flex: 1; height: 5px; background: var(--bg-overlay); border-radius: 3px; overflow: hidden; }
.gg-bar { height: 100%; border-radius: 3px; transition: width 0.3s; }
.gg-points { width: 28px; font-size: 0.78rem; }
.gg-points.hit  { color: var(--green-400); }
.gg-points.miss { color: var(--red-500); }
.gg-status { width: 16px; font-size: 0.78rem; }

/* Scenarios */
.scenarios-card { margin-top: 0; }
.scenarios-table { display: flex; flex-direction: column; gap: 0; }
.scen-head {
...
.scen-row:hover { background: var(--bg-elevated); }
.scen-row.target { background: var(--accent-subtle); }
.scen-upcoming { color: var(--text-secondary); }
.scen-cgpa { font-weight: 500; }
.scen-cgpa.high { color: var(--green-400); }
.scen-cgpa.mid  { color: var(--amber-400); }
.scen-cgpa.low  { color: var(--red-500); }
.scen-class { font-size: 0.78rem; color: var(--text-muted); }

@media (max-width: 900px) {
  .forecast-body { grid-template-columns: 1fr; }
  .current-card { gap: 16px; }
  .current-divider { display: none; }
  .current-stat { padding: 0; }
}
</style>
